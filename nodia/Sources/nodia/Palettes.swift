...
/// the panel background, and the selection wash painted over that background
...
/// backgrounds — and with margin to spare, because the glass samples whatever
/// is behind the window and the tint only pulls it partway back. Values tuned
/// to one assumed backdrop fell to ~3.4:1 whenever the panel happened to sit
/// over something pale.
///
...
///
/// `tools/contrast.py` recomputes all of it against each palette's own tint
/// color, which is a stand-in: the glass's real backdrop can't be read back, so
/// those numbers check the palette's internal relationships rather than what a
/// screenshot would measure. Run it when adding a palette.
enum Palettes {
    static let system = Palette(
        id: "system", name: "跟随系统", isDark: nil, tint: nil,
